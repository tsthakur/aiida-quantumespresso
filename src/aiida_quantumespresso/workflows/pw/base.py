...
class PwBaseWorkChain(ProtocolMixin, BaseRestartWorkChain):
    ...
    def validate_md_parameters(self):
        ...
        if self.inputs.get('previous_trajectory'):
            ...
            # mdsteps_todo will be -ve in that case and another iteration will not be launched
    ...
    def prepare_process(self):
        ...
        # The way velocities are calculated, drops the last step, so I run an additional step to make up for it
        self.ctx.inputs.parameters['CONTROL']['nstep'] = self.ctx.mdsteps_todo + 1
    ...
    def update_mdsteps(self):
        ...
        try:
            ...
            self.report(f'No output_trajectory was generated by {node.label}<{node.pk}>.')
            ...
            if not traj.get_extra('discard_trajectory', False):
                ...
                self.report(
                    f'{node.process_label}<{node.pk}> ran {nsteps_run_last_calc} steps ({self.ctx.mdsteps_done} done - {self.ctx.mdsteps_todo} to go).'
                )
                ...
                self.report(
                    f'{node.process_label}<{node.pk}> ran {nsteps_run_last_calc} steps. This trajectory will be DISCARDED!'
                )
    ...
    def handle_out_of_walltime(self, calculation):
        ...
        if self.ctx.inputs.parameters['CONTROL'].get('calculation', 'scf') == 'md':
            ...
            self.report_error_handled(
                calculation, 'MD calculation is restarting from the previous wavefunctions and charge densities'
            )
