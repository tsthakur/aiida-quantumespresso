...
class PwParser(BaseParser):
    ...
    def parse_stdout(self, parameters, parser_options=None, parsed_xml=None, crash_file=None):
        ...
        # output files are most likely corrupt and cannot be restarted from except in case of MD simulaiotns, where
    ...
    def build_output_trajectory(parsed_trajectory, structure):
        ...
        if not md_parameters: 
            ...
            # Then, it must be checked that forces exist
            try:
                # Converting forces back to atomic units for easier calculation of velocities
                forces_au = numpy.array(parsed_trajectory['forces']) * CONSTANTS.bohr_to_ang / CONSTANTS.ry_to_ev
            ...
            if calculate_velocities:
                ...
                masses = numpy.array([structure.get_kind(symbol).mass for symbol in symbols]).reshape(-1,1)  
                ...
                velocities_au = numpy.diff(positions_au[1:], axis=0) / dt - 0.5 * accelerations[:-1] * dt 
                last_step_velocity = velocities_au[-1] + 0.5 * (accelerations[-2] + accelerations[-1]) * dt
                velocities_au_complete = numpy.append(velocities_au, [last_step_velocity], axis=0)
                '''
                ...
                velocities_au_complete = numpy.insert(velocities_au, 0, numpy.zeros(velocities_au[0].shape), axis=0)
                ...
                for key, value in parsed_trajectory.items():
                    trajectory.set_array(key, numpy.array(value)[:-1])
